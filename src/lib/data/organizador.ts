 ...
 * metadata, color primario) del organizador actual. */
 ...
 */
export const obtenerOrganizadorActual = cache(async (): Promise<OrganizadorEmailInfo | null> => {
  ...
  const { data } = await supabase
    .from("organizadores")
    .select("id, nombre, email_contacto, logo_url, color_primario")
    .eq("id", id)
    .maybeSingle();
  return data;
});
